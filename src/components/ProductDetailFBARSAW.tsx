...
import { FBARProduct } from '../types/product';
import { fetchProductDetail } from '../api/products';
import ProductImg_fbarsaw from '@/assets/images/product/fbarsaw.jpg';
...
const keyToChineseMap: { [key: string]: string } = {
  ...
  process: '工艺',
  function: '功能',
  insertion_loss: '插入损耗(typ)',
  attenuation: '衰减量(typ)',
  isolation: '绝缘(typ)',
  input_power: 'Input Power(max)',
  impedance: '终端阻抗',
  dimensions: '尺寸(LxWxT)',
...
const ProductDetailFBARSAW: FC = () => {
  const { part_number } = useParams<{ part_number: string }>();
  const [product, setProduct] = useState<FBARProduct | null>(null);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (data && data.product_type === 'FBAR') {
            setProduct(data as FBARProduct);
        }
      } catch (error) {
    ...
    <div style={{ paddingBottom: '80px' }}>
      ...
      <div className="container" style={{ marginTop: 30 }}>
          ...
          <div className='' style={{
            ...
            <Flex vertical gap={12}>
              ...
              <Flex justify="space-between">
                ...
                {/* 假设 FBAR&SAW 产品有 spq 字段，若没有需要调整 */}
              </Flex>
            </Flex>
          </div>
        </Flex>
        ...
        <div style={{ marginTop: 24, textAlign: 'right' }}>
          <Link to="/products-fbarsaw">返回产品列表</Link>
        </div>
      </div>
    </div>
  );
...
export default ProductDetailFBARSAW;